...
;; -----------------------------------------------------------------------------
;; R7RS when and unless macros (Section 4.2.1)
;; -----------------------------------------------------------------------------
(define-macro (when test . body)
  "(when test body ...)

   R7RS macro that evaluates body expressions when test is true.
   Returns #void if test is false."
  `(if ,test
     (begin ,@body)))
...
(define-macro (unless test . body)
  "(unless test body ...)
   ...
   R7RS macro that evaluates body expressions when test is false.
   Returns #void if test is true."
  `(if (not ,test)
     (begin ,@body)))

;; -----------------------------------------------------------------------------
;; R7RS case macro (Section 4.2.1)
;; -----------------------------------------------------------------------------
(define-macro (case key . clauses)
  "(case key ((datum ...) expr ...) ... (else expr ...))
   (case key ((datum ...) => proc) ... (else => proc))

   R7RS case expression. Evaluates key, then matches against datums using eqv?.
   Evaluates and returns the expressions of the first matching clause.
   The else clause matches if no other clause matches.
   With => syntax, the procedure is called with the key value."
...
;; n-ary, take, drop - moved to bridge.ts
...
(define (constructor)
  ...
  (throw "Invalid call to constructor function"))
...
;; complement, always, once, flip - moved to bridge.ts
...
;; -----------------------------------------------------------------------------
;; R7RS let-values - binds multiple values returned by expressions
;; -----------------------------------------------------------------------------
(define-macro (let-values bindings . body)
  "(let-values (((var ...) expr) ...) body ...)

   R7RS macro for binding multiple values. Each binding clause has a list of
   variable names and an expression that returns multiple values."
  (if (null? bindings)
    `(begin ,@body)
...
;; -----------------------------------------------------------------------------
;; R7RS let*-values - sequential multiple value bindings
;; -----------------------------------------------------------------------------
(define-macro (let*-values bindings . body)
  "(let*-values (((var ...) expr) ...) body ...)

   R7RS macro for sequential binding of multiple values. Like let-values but
   each binding can refer to variables from previous bindings."
  (if (null? bindings)
    `(begin ,@body)
    (let* ((first-binding (car bindings))
            (vars (car first-binding))
...
;; Internal: current exception handler stack
...
(define (raise obj)
  "(raise obj)
   ...
   Raises a non-continuable exception with obj as the exception value.
   The current exception handler is called with obj. If the handler returns,
   the exception is re-raised."
  (if (null? *current-exception-handlers*)
    (%raise obj)  ;; Use %raise to preserve object type
    (let ((handler (car *current-exception-handlers*)))
      ;; Call handler, then re-raise if it returns (non-continuable)
      (handler obj)
      ;; If handler returned, that's an error for non-continuable exceptions
...
(define (raise-continuable obj)
  "(raise-continuable obj)
   ...
   Raises a continuable exception with obj as the exception value.
   The current exception handler is called with obj and its return value
   is returned to the continuation of the raise-continuable call."
  (if (null? *current-exception-handlers*)
    (%raise obj)  ;; Use %raise to preserve object type
    (let ((handler (car *current-exception-handlers*)))
      ;; For continuable exceptions, return the handler's result
      (handler obj))))
...
(define (with-exception-handler handler thunk)
  "(with-exception-handler handler thunk)
   ...
   Installs handler as the current exception handler and calls thunk.
   handler is a procedure of one argument (the exception).
...
(define (error message . irritants)
  "(error message irritant ...)
   ...
   Raises a non-continuable exception with an error object containing
   the message and irritants."
  (raise (apply make-error-object message irritants)))

;; -----------------------------------------------------------------------------
;; R7RS guard macro - exception handling with pattern matching
;; Uses LIPS's native try/catch since call/cc is not available
;; -----------------------------------------------------------------------------
(define-macro (guard clause-and-body . rest)
  "(guard (var clause ...) body ...)

   R7RS exception handling macro. Evaluates body and if an exception is raised,
   binds it to var and evaluates the cond-style clauses. If no clause matches
   and there's no else clause, the exception is re-raised."
