...
import { env as globalEnv } from "./stdlib.js";
...
const catchEither = (fn: () => any) => {
  try {
    return new Pair(fn(), nil);
  } catch (error: any) {
    return new Pair(nil, error?.message ?? error);
  }
};
...
export const RAMDA_FUNCTIONS = {
  ...
  add: (a, b) => {
    ...
    return (globalEnv.get("+") as Function)(a, b);
  },
  subtract: (a, b) => {
    if (typeof a === "number" && typeof b === "number" && isSafeNumber(Number(a) - Number(b))) {
      return a - b;
  ...
  negate: (a) => {
    ...
    return (globalEnv.get("-") as Function)(a);
  },
