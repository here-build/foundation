(test.skip "continuations: don't call after call continuation"
      (lambda (t)
        (let ((x #f))
...
(test.skip "continuations: calling continuation"
      (lambda (t)
        (t.plan 1)
        (let ((x #f))
...
(test.skip "continuations: make-range"
              (lambda (t)
...
(test.skip "continuations: saving/restoring environment"
      (lambda (t)
...
(test.skip "continuations: scope mutation"
      (lambda (t)
        (define counter '())
...
(test.skip "continuations: coroutine generator"
      (lambda (t)
