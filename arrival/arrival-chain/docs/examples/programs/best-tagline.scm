...
;;
;; Wiring (best-tagline.example.json):
;;   files:
...
;;   env (per-run experimental knobs only — system prompts live in this
;;        program as constants since they define the algorithm, not the
;;        experiment):
;;     initial-tagline    the starting candidate
;;     max-iter           inner GEPA loop cap per branch (e.g. 8)
;;     plateau-delta      degrading threshold, typically 0.02 (i.e. need ≥2%)
;;     total-iter-cap     worklist branch cap (oscillation safety, e.g. 20)
;;     bounce-threshold   bounce-rate above which we triage and split (e.g. 0.50)
;;     pov-count          how many of the typed reflection POVs to run per branch (1..4)
...
(define (gepa-until-plateau initial personas hints sys)
  (define (loop tagline iter history)
    (let* ((reactions (reactions-of tagline personas))
      ...
      (cond
        ((>= iter project/max-iter)                  (best-of history+))
        ((degrading? history+ project/plateau-delta) (best-of history+))
...
(define results          (optimize-tagline project/initial-tagline initial-personas))
