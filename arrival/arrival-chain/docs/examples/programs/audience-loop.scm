...
;;
;; Wiring (via configs/audience-loop.example.json):
;;   files:
;;     personas.json    nested-shape (V's .data/profiles-*.json)
;;     variants.json    list of { id, lead, scenario, hero-lines } objects
;;   env:
;;     product-context  string used in each prompt
;;     min-replays      reactions per cell (e.g. 3)
;;     min-for-boundary skip boundary if fewer personas (e.g. 3)
;;
...
(define (product-header)
  (if (equal? project/product-context "") ""
...
(define (cell-reactions persona variant)
  (map (lambda (i) (react-cell persona variant i)) (range project/min-replays)))
...
(define (variant-boundary variant personas)
  (let ((cls (map (lambda (p) (list p (cell-classify p variant))) personas)))
    (if (< (length cls) project/min-for-boundary)
      #f
      (car (infer/chat "high"
             (list (infer/chat/system "Return only JSON.")
