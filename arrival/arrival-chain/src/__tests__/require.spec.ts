...
const seed = (project: Project, m: string, p: string, valueJson: string) => {
  project.upsertTask(m, p, null).result = new InferenceResult({ valueJson });
};
...
describe("require — Plexus VFS preamble", () => {
  ...
  it("threads required helpers into an infer call", async () => {
    ...
    seed(project, "fast", "Greet world", '"hi"');
